...
class App extends Component {
  ...
  render() {
    ...
    const filtereventList = destinationsList.filter(eachval =>
      eachval.name.toLowerCase().includes(userInput.toLowerCase()),
    )
    return (
      <div>
        ...
        />
        <ul>
          {filtereventList.map(each => (
            <DestinationSearch key={each.id} userDetails={each} />
          ))}
        </ul>
      </div>
    )
